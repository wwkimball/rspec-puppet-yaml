...
module RSpec::Puppet
  ...
  module Yaml
    # Provides methods that enable users to define rspec-puppet examples via
    # YAML.
    module Parser
      extend self
      extend RSpec::Puppet::Support

      def self.guess_type_from_path(value)
        guess_type_from_path(value)
      end
      ...
      def parse_rspec_puppet_yaml(yaml_file)
        ...
        apply_describe(
          ...
          { 'name' => get_eut_name(yaml_file, rspec_file),
            'type' => RSpec::Puppet::Yaml::Parser.guess_type_from_path(rspec_file)
          }
        )
      end
